  ...
  return (
    <PageFrame>
      <PageHero
        ...
        intro="KIC Ltd brings local knowledge and a long-term perspective to the systems that support a more prosperous Tanzania."
        ...
        <div className="grid border-t border-border sm:grid-cols-2 lg:grid-cols-3">
          {businessAreas.map((area, index) => (
            <Link
              ...
              key={area.slug}
              className="group border-b border-border p-8 transition-colors hover:bg-secondary"
            >
              <span className="text-sm font-mono text-accent">0{index + 1}</span>
              <h2 className="mt-16 text-2xl font-medium tracking-tight group-hover:text-accent">
                {area.title}
              </h2>
              <p className="mt-4 leading-relaxed text-muted-foreground">{area.text}</p>
              <ArrowUpRight
                className="mt-12 text-accent transition-transform group-hover:translate-x-1 group-hover:-translate-y-1"
                size={20}
              />
            </Link>
          ))}
